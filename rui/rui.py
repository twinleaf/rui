...
def _parser_setup(
    ...
    *,
    ...
    p.add_argument("-s", "--sensor", default="/", help="Sensor path in the sensor tree")
    p.add_argument("--test", action="store_true", help=argparse.SUPPRESS)

    # optional flags
    if "a" in flags:
        p.add_argument(
            "-a", "--all", action="store_true", help="Select all matched RPCs"
        )
    if "e" in flags:
        p.add_argument(
            "-e",
    ...
    if "p" in flags:
        p.add_argument(
            "-p", "--peek", action="store_true", help="Don't prompt to change RPC value"
        )
    if "c" in flags:
        p.add_argument(
            "cli_args",
            nargs="*",
            metavar="search terms [+arg]",
...
def rui_parse_args() -> argparse.Namespace:
    ...
    remove_parser = cache_subparsers.add_parser(
        "remove", help="Remove device RPC cache"
    )
    ...
    if sys.argv[1:] and sys.argv[1] == "cache":
        if not sys.argv[2:] or sys.argv[2] not in {
            "print",
            "remove",
            "-h",
            "--help",
