...
def gui(dev, args):
    ...
    if args.terms:
        selected = search_select(
            client.list, args.terms, args.exact, args.all, args.multisearch
        )
    elif args.restore:
        selected = []
    ...
    try:
        subprocess.run(["i3-msg", "floating", "toggle"], capture_output=True)
...
class MainWindow(QWidget):
    ...
    def display_rpc_slider(self):
        ...
        if text := self.tool_bar.text():
            ...
            idx = next(
                (i for i, rpc in enumerate(self.rpcs_displayed) if rpc.name == value),
                None,
            )
            if idx is not None:
                ...
                self.slider_configs.update_displayed_rpcs(
                    self.rpc_list[index].name, min_val, max_val, 1
                )
    ...
    def get_rpc_min_max(self, rpc):
        ...
        max_val = max(
            current_value, rpc.to_arg_type(self.slider_configs.get_rpc_max(rpc.name))
        )
