...
def _select_input(matched: RPCList) -> RPCList:
    ...
    if matched.lonely():
        ...
        matched.print()
        while not (answer := input(SELECT_PROMPT)) or answer.isspace():
            pass
        print()  # spacer
        match answer:
            case "*":
                return matched
            case nums if nums and all(
                [n.isnumeric() and 1 <= int(n) <= len(matched) for n in nums.split()]
            ):
...
SELECT_ERR = lambda l: (
    f"Invalid. Select number(s) from 1 to {len(l)} or enter terms to narrow search."
)
...
CURRENT_VAL_MSG = lambda a, v: (
    f"Previous value: {v}" if a is not None else f"Current value: {v}"
)
